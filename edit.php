...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $qu = "SELECT * FROM brand WHERE id ='$id'";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $brand = $_POST['name'];
    $idd = $_POST['id'];
    $upd = "UPDATE brand SET name = '$brand' WHERE id = '$idd'";
    $res3 = mysqli_query($conn, $upd);
    if($res3){
      header("location:./brandInsert.php?from=editBrand");
    }
}
?>
<!doctype html>
...
<head>
  ...
  <title>Edit Brand - Digital Shelf</title>
...
<script>
 ...
 <div class="card">
  ...
  <div class="mb-3">
    ...
    <input type="text" class="form-control" value="<?php echo $res2['name']?>" name="name" id="">
  </div>
