...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if($res){
        ...
        if ($row) {
            $user_id = $row['id'];
            $query = "INSERT INTO userprofiles (user_id, first_name, last_name, phone_number, date_of_birth, address) 
                      VALUES ('$user_id','$firstName', '$lastName', '$phone', '$dob','$address')";
      ...
      <form method="post" enctype="multipart/form-data" style="padding:40px;">
        ...
        <div class="mb-3">
          ...
          <input
            type="text"
            name="address"
            placeholder="House No. 123, Street No. 45"
            class="form-control"
          />
        </div>
