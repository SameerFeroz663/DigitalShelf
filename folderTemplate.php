...
if(isset($parts[1])){
    ...
    $co = mysqli_query($conn,$sel);
    $fetch = mysqli_fetch_assoc($co);
    $sel2 = "SELECT * FROM FolderName WHERE subFolder = '$folder'";
    $conn1 = mysqli_query($conn,$sel2);
    $fetch1 = mysqli_fetch_assoc($conn1);
    $pathIncom = "../".$folder;
    if(is_dir($pathIncom)){
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    if (icon && searchContainer) {
      ...
      <div class="body-wrapper-inner">
        <div class="container-fluid">
          <!--  Row 1 -->
         <div class="card p-5">
             <?php $folder = urldecode($parts[2]);?>
             <h2><?php echo $parts[0]. '/' . $fetch['name'] . '/' . $folder . '/..';?></h2>
             <a href='../'><div id='dirs'>Back <i class="ti ti-arrow-left" style="position:absolute;"></i></div></a>
               <?php
              if (!empty($msg)) {
    echo "<div>" . htmlspecialchars($msg) . "</div>";
             ...
             foreach($path as $items){
                ...
                if(is_dir($itemPath)){
                    echo "<a  href='$itemPath/index.php'><div id='dirs' class='file-item' data-file='$items'>
" . $items . "<br><i class='ti ti-folder'></i></div></a>";
                }
            }
