...
<head>
  ...
  <title>Manage Users - Digital Shelf</title>
...
<div style="max-height: 400px; overflow-y: auto; border-radius: 15px;">
  <table class="table" id="example" style="width:100%; max-height: 400px; overflow-y: auto; border-radius: 15px;">
  <thead>
    <tr>
      <th>#</th>
      <th>Username</th>
      <th>Name</th>
      <th>Email</th>
      <th>No Of Brands Assigned</th>
      <th style='width:8em;'>Created</th>
      <th style='width:8em;'>Updated</th>
      <th>Action</th>
    </tr>
  </thead>
...
SELECT 
  ...
  u.created_at,
  u.updated_at,
  COUNT(DISTINCT ub.brand_id) AS brands
FROM users AS u
JOIN userprofiles AS up ON u.id = up.user_id
LEFT JOIN UserBrands AS ub ON ub.user_id = u.id
WHERE u.id NOT IN (
  SELECT user_id FROM userroles WHERE role_id = 2
  )
GROUP BY 
  u.id, u.username, up.first_name, u.email, u.created_at, u.updated_at
ORDER BY u.id;
...
$res = mysqli_query($conn, $query);
    ...
    while($row = mysqli_fetch_assoc($res)) {
      ...
      <tr style='border-radius: 15px;'>
        ...
        echo"
        <td>
                  <a href='./editUser.php?id={$row['id']}'><i class='ti ti-edit text-primary'></i></a>

          <a href='./deleteuser.php?id={$row['id']}'><i class='ti ti-trash'></i></a>
        </td>
      </tr>
      ";
    }
