...
<div style="max-height: 400px; overflow-y: auto; border-radius: 15px;">
  <table class="table" style="width:100%; max-height: 400px; overflow-y: auto; border-radius: 15px;">
  <thead>
    <tr>
      <th>#</th>
      <th>Username</th>
...
GROUP BY 
  u.id, u.username, up.first_name, u.email, up.address, u.created_at, u.updated_at
ORDER BY u.id;
...
$res = mysqli_query($conn, $query);
    ...
    while($row = mysqli_fetch_assoc($res)) {
      ...
      <tr style='border-radius: 15px;'>
        ...
        echo"
        <td>
          <a href='./deleteuser.php?id={$row['id']}'><button style='width:7em;' class='btn bg-danger text-light btn-sm'>Delete</button></a>
        </td>
      </tr>
      ";
    }
